...
def setup_argparser() -> argparse.ArgumentParser:
    ...
    # --- Added for valgrind integration ---
    parser.add_argument("--valgrind", action="store_true", help="run C tests under valgrind")
    # --------------------------------------
    return parser
...
# ----------- MODIFIED execute() WITH VALGRIND SUPPORT ----------------
def execute(config, test_type, input_file=None, use_valgrind=False) -> subprocess.CompletedProcess[str]:
    match test_type:
        ...
        case TestType.C:
            ...
            if use_valgrind:
                valgrind_args = [
                    "valgrind",
                    "--leak-check=full",
                    "--error-exitcode=99",  # nonzero if leaks found
                    "--log-fd=2",          # output valgrind log to stderr
                    # "--quiet",           # Comment out to see full reports, or keep to silence valgrind
                ]
...
def run_test(tests_dir: Path, config: dict[str, str], test_type: TestType, use_valgrind=False):
    success = True

    input_path = tests_dir / Path(f"input_{config['idx']}.txt")
    output_path = tests_dir / Path(f"output_{config['idx']}.txt")
    with input_path.open() as input_file:
        result = execute(config, test_type, input_file, use_valgrind=use_valgrind)
    ...
    if result.stderr and not use_valgrind:
        success = False
...
def run_negative_test(
    ...
    if result.returncode == 99:
        ...
        print_white_on_red("Memory leak detected by valgrind!")
        print_white_on_red(result.stderr)
...
def main():
    ...
    for config in configs:
        ...
        for test_type in test_types:
            ...
            run_test(tests_dir, config, test_type, use_valgrind=(args.valgrind and test_type == TestType.C))
        print()
    ...
    if args.test_python and args.test_c:
        ...
        for label, input_data in (("random", random_input), ("dummy", dummy_input)):
            ...
            c_run = execute({"k": "3"}, TestType.C, input_data, use_valgrind=args.valgrind)
            input_data.seek(0)
            ...
            if c_run.returncode == 99:
                print_white_on_red("Memory leak detected by valgrind!")
                print_white_on_red(c_run.stderr)
                skip = True
            elif c_run.returncode or c_run.stderr and not args.valgrind:
    ...
    for test_type, applicable in (
        ...
        result = execute(_config, test_type, dummy_input, use_valgrind=(args.valgrind and test_type == TestType.C))
        dummy_input.seek(0)
        ...
        if result.stderr and not args.valgrind:
            success = False
            print("process had non-empty stderr:")
            print_white_on_red(result.stderr)
    ...
    for invalid_param in INVALID_PARAMETERS_VALUES:
        ...
        for test_type, applicable in (
            ...
            run_negative_test(
                ...
                dummy_input,
                use_valgrind=(args.valgrind and test_type == TestType.C),
            )
            dummy_input.seek(0)
    print()
